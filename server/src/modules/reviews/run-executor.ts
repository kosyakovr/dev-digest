...
export class ReviewRunExecutor {
      ...
      for (const { runId, agent } of jobs) {
        await this.repo
          .completeAgentRun(runId, {
            ...
            tokensIn: 0,
            tokensOut: 0,
            // Pre-work failure: no LLM call was made. null = "unknown", not "free".
            costUsd: null,
            findingsCount: 0,
  ...
  private async runOneAgent(
    ...
    try {
      ...
      // L02 — skills. The agent's attached skills are ordered markdown blocks;
      // `linkedSkills` already returns them ORDER BY "order" ASC, and that array
      // order is the order they appear in the assembled prompt. Two independent
      ...
      await this.repo
        .completeAgentRun(runId, {
          ...
          tokensIn: 0,
          tokensOut: 0,
          // The outcome never came back, so any spend so far is unrecoverable
          // here: record "unknown" rather than claiming the run was free.
          costUsd: null,
          findingsCount: 0,
          grounding: '0/0 passed',
          error: msg,
