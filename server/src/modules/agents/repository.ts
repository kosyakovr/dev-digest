...
/** A skill linked to an agent (with its order + per-link toggle), joined from
 *  agent_skills. `enabled` here is the LINK's flag; `skill.enabled` is the
 *  skill's own. A skill reaches the prompt only when both are true. */
...
/** One entry of the ordered set an agent's Skills tab submits. */
...
export class AgentsRepository {
   ...
   *
   * Delete-then-insert inside ONE transaction: the agent's linked set is read by
   * every review run, and a crash between the two statements would otherwise
   * leave the agent with no skills at all.
   */
