...
export const ExtractionSchema = z.object({
  candidates: z.array(
    z.object({
       ...
       * classification and the score come LAST, after the rule and its evidence
       * are written. Moving `category` first makes the model commit to a label
       * before it knows what it is about to say — a measured scan collapsed to
       * one category and a flat confidence for every candidate.
       */
      category: z.enum([
        'naming',
        'structure',
      ...
      /**
       * How many sampled files show the pattern. Deliberately NOT persisted: it
       * exists to make the model count before it scores, and a self-reported
...
/** Schema name the provider tags the call with (and tests key fixtures on). */
...
Scores must discriminate: if every candidate carries the same confidence, you have not scored them, and the weak ones will waste the reviewer's time.
