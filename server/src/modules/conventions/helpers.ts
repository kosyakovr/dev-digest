 ...
 * from the accepted ones. No IO, no model, no DB — nothing here awaits.
 */
...
export type DropReason = 'empty_rule' | 'unknown_file' | 'snippet_too_short' | 'snippet_not_found';
...
export function verifyCandidate(files: Map<string, SampledFile>, c: RawCandidate): VerifyResult {
  ...
  const snippetLines = (c.evidence_snippet ?? '')
    .split('\n')
    .map((l) => l.replace(/^\s*\d+\t/, '')) // the model sometimes echoes our gutter
  ...
  const snippet = dedent(file.lines.slice(idx, idx + span))
    .join('\n')
    .trimEnd();
 ...
 * the line nearest the model's own claim, so a repeated line (`}`, an import)
...
function findLine(lines: string[], head: string, claimed: number | null): number {
  ...
  const first = hits[0] as number;
  if (claimed == null) return first;
...
/**
 * Drop candidates repeating a rule already in `seen` (the decisions earlier
 * scans left behind) or each other. First occurrence wins, so the
 * highest-confidence one survives when the caller sorts before calling.
 */
...
/**
 * Assemble the selected candidates into a skill draft. The body is markdown the
...
export function buildSkillDraft(repoFullName: string, rows: ConventionRow[]): ConventionSkillDraft {
  const name = `${repoSlug(repoFullName)}-conventions`;
  const sections = rows.map((r) => {
    ...
    if (r.evidencePath) {
      lines.push(
        '',
        `Detected in \`${r.evidencePath}:${r.evidenceLine}\`:`,
        '',
        '```',
        r.evidenceSnippet ?? '',
        '```',
