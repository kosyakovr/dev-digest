...
| `02-repo-pulls-detail` | PR list → cost + findings column headers → open PR #482 → review detail route |
| `03-agents` | agents list renders the seeded reviewer agents |
| `04-pr-findings` | PR #482 → Agent runs tab → seeded run verdict + findings; severity counters; severity filter on/off |
...
| `08-skills` | skills list → open a skill → Config/Preview/Versions tabs → agent editor's Skills tab |
